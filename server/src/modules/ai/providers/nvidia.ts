...
export class NvidiaProvider implements AiProvider {
  ...
  async chat(
    ...
    try {
      ...
      return {
        ...
        metadata: {} as ChatResponse['metadata'],
      };
    }
  }

  async evaluatePractice(word: string, sentence: string): Promise<PracticeEvaluation> {
    if (!env.NVIDIA_API_KEY) {
      return {
        isCorrect: true,
    ...
    const data = await this.request({
      ...
      messages: [{ role: 'system', content: PRACTICE_PROMPT }, { role: 'user', content: userMessage }],
      temperature: 0.5,
      max_tokens: 300,
      response_format: { type: 'json_object' },
    });
    ...
    try {
      return JSON.parse(data.choices[0]!.message.content) as PracticeEvaluation;
    } catch {
      return {
        isCorrect: false,
  ...
  async evaluateJournal(content: string, prompt: string): Promise<JournalFeedback> {
    ...
    const data = await this.request({
      ...
      messages: [{ role: 'system', content: JOURNAL_PROMPT }, { role: 'user', content: userMessage }],
      temperature: 0.3,
      max_tokens: 1000,
      response_format: { type: 'json_object' },
    });
    ...
    try {
      return JSON.parse(data.choices[0]!.message.content) as JournalFeedback;
    } catch {
